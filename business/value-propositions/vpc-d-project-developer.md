...
|-----|------|------------|-------|
| Source materials at the right price to stay within project budget | Functional | Critical | Budget overruns are the primary risk |
| Find suppliers quickly to keep projects on schedule | Functional | High | Delays in sourcing cascade into delivery delays |
| Ensure materials meet building regulations and specifications | Functional | High | Compliance is a legal requirement; defects are costly |
| Compare suppliers to avoid overpaying or accepting poor quality | Functional | High | Done informally today; rarely structured |
| Track supplier performance across projects | Functional | Medium | Often not done systematically; lessons don't transfer |
| Manage multiple concurrent procurement needs across projects | Functional | Medium | Ad hoc today; no central visibility |
| Reduce the time project managers spend on procurement logistics | Functional | Medium | Procurement is a distraction from core development work |
| Avoid material quality issues that create defect claims post-completion | Emotional | High | Post-completion defects are expensive and reputationally damaging |
...
| Limited procurement expertise within the team | Medium | No dedicated buyer; mistakes are made through inexperience |
...
|---|---|
| Price volatility and no benchmarking | Comparison report surfaces market pricing across multiple qualified suppliers |
| No central visibility across projects | Unified procurement search across all projects in the system |
| Supplier reliability hard to assess | Supplier reliability scoring based on historical performance data |
| Same suppliers used by habit | Specification matching surfaces alternatives the team doesn't know |
| No record of past performance | Model trained on historical order and performance data builds institutional memory |
| Specification checking is manual | Specification-to-material matching validates compliance automatically |

| Customer Gain | Our Gain Creator |
|---|---|
| Price benchmarking | Multi-supplier comparison with pricing as a primary scoring dimension |
| Faster supplier selection | Automated matching from specification to ranked supplier shortlist |
| Supplier reliability scores | Historical performance data integrated into supplier ranking |
| Consistent compliance checking | Specification matching aligned to building regulations and cert requirements |
| Portfolio visibility | All projects and procurement events in one searchable interface |
| Accessible to non-specialists | AI-guided interface lowers the expertise required to run a good procurement |
...
|-----------|-------|-------|
| Pain severity | ★★★☆☆ | Real pain, but less acute than segments A and B |
...
- **Product simplification:** This segment likely doesn't need local infrastructure or fine-tuning in v1. A lighter hosted product tier with a shared base model could serve them at lower cost and shorter onboarding.
- **Data challenge:** Without an ERP, onboarding data will be messy. Procurement data cleanup service is a relevant upsell for this segment.
- **Entry point:** Project developers often work with construction companies. A warm referral from a construction company customer (Segment A) is the most efficient route to this segment.

---

## Qualification Criteria

A project developer is a good fit if they meet:
- [ ] Active build programme (multiple concurrent projects)
- [ ] Some structured procurement history (even Excel-based)
- [ ] Experiencing budget overruns or supplier reliability problems on recent projects
- [ ] Decision-maker accessible and willing to trial a new tool
- [ ] Open to a lighter hosted product tier (if local deployment is not required)
